...
/**
 * One-time cleanup: app_settings, smtp_settings, badge_definitions, and challenge_definitions
 * were all replaced by global_configuration (see {@code ConfigurationDomainService},
 * {@code BadgeDefinitionRepository}, {@code ChallengeDefinitionRepository}), but Hibernate's
 ...
 *
 * <p>badge_definitions/challenge_definitions get one extra step first: any rows already in them
 * (including ones an admin added or edited beyond the seeded defaults) are copied into
 * global_configuration before the old table is dropped, so dropping it can't silently lose data.
 * {@code @Order} makes sure this migration runs before {@code ReferenceDataSeeder}, whose
 * count()==0 seeding check would otherwise re-seed just the defaults on top of an empty store.
 */
@Component
@Slf4j
@Order(0)
